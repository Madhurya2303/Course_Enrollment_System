...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // ✅ Save auth headers and user info
      localStorage.setItem("access-token", token);
      localStorage.setItem("client", client);
      localStorage.setItem("uid", uid);
      localStorage.setItem("user-name", data.data.name);
      localStorage.setItem("user-id", data.data.id); // required for per-user state
...
// export default function Login() {
//   const { role } = useParams(); // 'student', 'instructor', or 'admin'
//   const [email, setEmail] = useState("");
//   const [password, setPassword] = useState("");
//   const [message, setMessage] = useState("");
//   const navigate = useNavigate();
...
//     // For students, allow login without password
//     if (role === "student" && !password) {
//       try {
//         const res = await fetch("http://localhost:3000/auth/sign_in", {
//           method: "POST",
//           headers: { "Content-Type": "application/json" },
//           body: JSON.stringify({ email, password: "dummy_password" }),
//         });
...
//     // Instructors and admins need password
//     try {
//       const res = await fetch("http://localhost:3000/auth/sign_in", {
//         method: "POST",
//         headers: { "Content-Type": "application/json" },
